...
1. فقط و فقط از اطلاعات موجود در گزارش بالا استفاده کن.
2. هیچ مقدار عددی جدیدی تولید نکن. اگر usage_percent در گزارش 73 است، هرگز آن را 100 یا مقدار دیگری ننویس.
3. فقط componentهایی را در خروجی استفاده کن که واقعاً در گزارش وجود دارند.
4. نام سرویس‌ها را تغییر نده و سرویس جدیدی اختراع نکن.
5. اگر یک مورد severity=HIGH یا CRITICAL دارد، نمی‌توانی آن را نادیده بگیری یا آن را از detailed_findings حذف کنی.
6. اگر برای یک نتیجه اطلاعات کافی وجود ندارد، حدس نزن؛ آن مورد را با عدم قطعیت مناسب توضیح بده.
7. متن matched_lines را تغییر نده و از آن برای ساختن اطلاعاتی که در گزارش وجود ندارد استفاده نکن.
8. قبل از تولید JSON، پاسخ خودت را با داده‌های گزارش تطبیق بده.
